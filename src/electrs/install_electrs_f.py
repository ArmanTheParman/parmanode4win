...
def start_electrs_docker():
    ...
    try:
        subprocess.run("docker ps | grep electrs", check=True, shell=True, capture_output=True).returncode 
    except:
        try: subprocess.run("docker start electrs", shell=True, check=True, capture_output=True) 
        except Exception as e: input(e)

    #start processes in the container
    try:
        subprocess.run(["docker", "exec", "-du", "root", "electrs", "bash", "-c", "/home/parman/parmanode/electrs/target/release/electrs --conf /home/parman/.electrs/config.toml >> /home/parman/.electrs/run_electrs.log 2>&1"], check=True)
    except Exception as e: input(e)
    try:
